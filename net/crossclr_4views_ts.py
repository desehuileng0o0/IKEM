...
class CrosSCLR(nn.Module):
    ...
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768,
        ...
        if not self.pretrain and not self.teacher_student:
            ...
            self.encoder_q_acceleration = base_encoder(in_channels=in_channels, hidden_channels=hidden_channels,
                                               ...
                                               edge_importance_weighting=edge_importance_weighting,
                                               **kwargs)
            self.encoder_student = base_encoder(in_channels=in_channels, hidden_channels=hidden_channels,
                                               hidden_dim=hidden_dim, num_class=feature_dim,
                                               dropout=dropout, graph_args=graph_args,
                                               edge_importance_weighting=edge_importance_weighting,
                                               **kwargs)
            self.views_fusion = nn.Sequential(nn.Linear(12, 12),
                                            nn.Linear(12, 12),
                                            nn.ReLU(),
                                            nn.Linear(12, 12),
                                            nn.ReLU(),
            ...
            if mlp:  # hack: brute-force replacement在每个全连接前又加了一层线性层
                ...
                self.encoder_q_acceleration.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp),
                                                       nn.ReLU(),
                                                       self.encoder_q_acceleration.fc)
                self.encoder_student.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp),
                                                       nn.ReLU(),
                                                       self.encoder_student.fc)

            for param_q in self.encoder_q.parameters():#将可迭代的对象打包成一个个元组
            ...
            for param_q in self.encoder_q_bone.parameters():
                param_q.requires_grad = False#初始化key的参数并且关闭梯度
            ...
            self.register_buffer("queue_teacher", torch.randn(feature_dim, self.K))#模型训练时不会更新，quene里现在存的都是随机数
    ...
    def forward(self, im_q, im_k=None, frame=None, view='all', cross=False, topk=1, context=True): 
        ...
        if not self.pretrain and self.teacher_student:
            #le student
            if view == 'all':
                im_viewx = torch.cat((im_q, im_q_motion, im_q_bone, im_q_acceleration), 1)
                im_viewx = im_viewx.permute(0, 2, 3, 4, 1).contiguous()
                im_viewx = self.views_fusion(im_viewx)
                im_viewx = im_viewx.permute(0, 4, 1, 2, 3).contiguous()
                return self.encoder_student(im_viewx)
            else:
                raise ValueError
    ...
    def cross_training(self, im_q, im_k, frame, topk=1, context=True):
        ...
        if self.pretrain and self.teacher_student:
            ...
            teacher = (q + q_motion + q_bone + q_acceleration) / 4.
            teacher = F.normalize(teacher, dim=1)
            #calculate student feature
            im_viewx = torch.cat((im_k, im_k_motion, im_k_bone, im_k_acceleration), 1)
            im_viewx = im_viewx.permute(0, 2, 3, 4, 1).contiguous()
            im_viewx = self.views_fusion(im_viewx)
            im_viewx = im_viewx.permute(0, 4, 1, 2, 3).contiguous()
            student = self.encoder_student(im_viewx)
            ...
            l_pos_viewx = torch.einsum('nc,nc->n', [teacher, student]).unsqueeze(-1)
            ...
            return logits_jx, logits_mx, logits_bx, logits_ax, pos_mask_jx, pos_mask_mx, pos_mask_bx, pos_mask_ax
        ...
        return logits_jm, logits_jb, logits_mj, logits_mb, logits_bj, logits_bm, logits_ja, logits_aj, logits_ma, logits_am, logits_ba, logits_ab, pos_mask_jm, pos_mask_jb, pos_mask_mj, pos_mask_mb, pos_mask_bj, pos_mask_bm, pos_mask_ja, pos_mask_aj, pos_mask_ma, pos_mask_am, pos_mask_ba, pos_mask_ab
