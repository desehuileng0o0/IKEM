...
class CrosSCLR_4views_Processor(PT_Processor):
    """
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label, frame in loader:#crop里面有random，后面再看，一个输入k一个输入q
            ...
            if epoch <= self.arg.cross_epoch:#没到cross的时候
                output, output_motion, output_bone, output_acceleration, target = self.model(data1, data2, frame)#一个是q的输入，一个是k的输入
                ...
                output_jm, output_jb, output_mj, output_mb, output_bj, output_bm, output_ja, output_aj, output_ma, output_am, output_ba, output_ab, mask_jm, mask_jb, mask_mj, mask_mb, mask_bj, mask_bm, mask_ja, mask_aj, mask_ma, mask_am, mask_ba, mask_ab = self.model(data1, data2, frame, cross=True, topk=self.arg.topk, context=self.arg.context)
                if hasattr(self.model, 'module'):
