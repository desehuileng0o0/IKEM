...
class SkeletonCLR_Viewsfusion_Processor(PT_Processor):
    """
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label, frame in loader:
            ...
            if self.arg.view == 'joint':
                ...
                bone1[:, 0:3, :, 20, :] = F.normalize(torch.cross((bone1[:, 0:3, :, 1, :]-bone1[:, 0:3, :, 8, :]), (bone1[:, 0:3, :, 4, :]-bone1[:, 0:3, :, 8, :])))
                bone2[:, 0:3, :, 20, :] = F.normalize(torch.cross((bone2[:, 0:3, :, 1, :]-bone2[:, 0:3, :, 8, :]), (bone2[:, 0:3, :, 4, :]-bone2[:, 0:3, :, 8, :])))
                print(bone1[0, 0:3, 0, 20, 0])
                print(bone1[1, 0:3, 1, 20, 0])
                data1 = bone1
                data2 = bone2
                ...
                Bone = [(1, 2), (2, 21), (3, 21), (4, 3), (5, 21), (6, 5), (7, 6), (8, 7), (9, 21),
                        (10, 9), (11, 10), (12, 11), (13, 1), (14, 13), (15, 14), (16, 15), (17, 1),
                        (18, 17), (19, 18), (20, 19), (21, 21), (22, 8), (23, 8), (24, 12), (25, 12)]
                Axis = [(1, 2), (2, 21), (3, 21), (4, 3), (5, 21), (6, 5), (7, 6), (8, 7), (9, 21),
                        (10, 9), (11, 10), (12, 11), (13, 1), (14, 13), (15, 14), (16, 15), (17, 1),
                ...
                bone1[:, 0:3, :, 20, :] = F.normalize(torch.cross((bone1[:, 0:3, :, 1, :]-bone1[:, 0:3, :, 8, :]), (bone1[:, 0:3, :, 4, :]-bone1[:, 0:3, :, 8, :])))
                bone2[:, 0:3, :, 20, :] = F.normalize(torch.cross((bone2[:, 0:3, :, 1, :]-bone2[:, 0:3, :, 8, :]), (bone2[:, 0:3, :, 4, :]-bone2[:, 0:3, :, 8, :])))
                bone1 = F.normalize(bone1,dim=1)
                bone2 = F.normalize(bone2,dim=1)
                #cross没有指定维度，默认了第一个尺度为3的维
                ra1 = torch.zeros_like(data1)
                ra2 = torch.zeros_like(data2)
                theta1 = torch.zeros_like(data1)
                theta2 = torch.zeros_like(data2)

                for b1, b2 in Axis:
                    ra1[:, 0:3, :, b1 - 1, :] = torch.cross(bone1[:, 0:3, :, b1 - 1, :],bone1[:, 0:3, :, b2 - 1, :])
                    ra2[:, 0:3, :, b1 - 1, :] = torch.cross(bone2[:, 0:3, :, b1 - 1, :],bone2[:, 0:3, :, b2 - 1, :])
                    #theta1[:, 0, :, b1 - 1, :] = torch.acos(torch.dot(bone1[:, 0:3, :, b1 - 1, :],bone1[:, 0:3, :, b2 - 1, :]) / (torch.norm(bone1[:, 0:3, :, b1 - 1, :])*torch.norm(bone1[:, 0:3, :, b2 - 1, :])))
                    #theta2[:, 0, :, b1 - 1, :] = torch.acos(torch.dot(bone2[:, 0:3, :, b1 - 1, :],bone2[:, 0:3, :, b2 - 1, :]) / (torch.norm(bone2[:, 0:3, :, b1 - 1, :])*torch.norm(bone2[:, 0:3, :, b2 - 1, :])))
                for b1, b2 in Axis:    
                    theta1[:, 0, :, b1 - 1, :] = torch.atan2(torch.norm(ra1[:, 0:3, :, b1 - 1, :],dim=1),torch.einsum('abcd,abcd->acd',bone1[:, 0:3, :, b1 - 1, :],bone1[:, 0:3, :, b2 - 1, :]))
                    theta2[:, 0, :, b1 - 1, :] = torch.atan2(torch.norm(ra1[:, 0:3, :, b1 - 1, :],dim=1),torch.einsum('abcd,abcd->acd',bone2[:, 0:3, :, b1 - 1, :],bone2[:, 0:3, :, b2 - 1, :]))
                #下面当下草稿纸
                #theta1[:, 0, :, b1 - 1, :] = torch.acos(torch.einsum('abcd,abcd->acd',bone1[:, 0:3, :, b1 - 1, :],bone1[:, 0:3, :, b2 - 1, :]))

                ran1 = F.normalize(ra1,dim=1)
                ran2 = F.normalize(ra2,dim=1)
                av1 = torch.zeros_like(data1)
                av2 = torch.zeros_like(data2)
                for i in range(3):
                    av1[:, i, :-1, :, :] = (theta1[:, 0, 1:, :, :] - theta1[:, 0, :-1, :, :]) * ran1[:, i, :-1, :, :]
                    av2[:, i, :-1, :, :] = (theta2[:, 0, 1:, :, :] - theta2[:, 0, :-1, :, :]) * ran2[:, i, :-1, :, :]
            ...
            elif self.arg.view == 'all':
                pass
            else:
                raise ValueError

            # forward
            output, target = self.model(data1, data2, frame, view=self.arg.view)
