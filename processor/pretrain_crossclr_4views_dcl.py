...
class CrosSCLR_4views_DCL_Processor(PT_Processor):
    """
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label, frame in loader:#crop里面有random，后面再看，一个输入k一个输入q
            ...
            if epoch <= self.arg.cross_epoch:#没到cross的时候
                output, output_motion, output_bone, output_acceleration, target,\
                    output_j_featuredim, output_m_featuredim, output_b_featuredim, output_a_featuredim, target_featuredim = self.model(data1, data2, frame)#一个是q的输入，一个是k的输入
                if hasattr(self.model, 'module'):
                    self.model.module.update_ptr(output.size(0))
                else:
                    self.model.update_ptr(output.size(0))
                loss = self.loss(output, target) + self.loss(output_j_featuredim, target_featuredim)
                loss_motion = self.loss(output_motion, target) + self.loss(output_m_featuredim, target_featuredim)
                loss_bone = self.loss(output_bone, target) + self.loss(output_b_featuredim, target_featuredim)
                loss_acceleration = self.loss(output_acceleration, target) + self.loss(output_a_featuredim, target_featuredim)
                ...
                loss = loss + loss_motion + loss_bone +loss_acceleration
            else:
                output_jm, output_jb, output_mj, output_mb, output_bj, output_bm, output_ja, output_aj, output_ma, output_am, output_ba, output_ab, \
                    mask_jm, mask_jb, mask_mj, mask_mb, mask_bj, mask_bm, mask_ja, mask_aj, mask_ma, mask_am, mask_ba, mask_ab,\
                        output_j_featuredim, output_m_featuredim, output_b_featuredim, output_a_featuredim, target_featuredim = self.model(data1, data2, frame, cross=True, topk=self.arg.topk, context=self.arg.context)
                if hasattr(self.model, 'module'):
                ...
                loss = loss.mean() + self.loss(output_j_featuredim, target_featuredim)
                loss_motion = loss_motion.mean() + self.loss(output_m_featuredim, target_featuredim)
                loss_bone = loss_bone.mean() + self.loss(output_b_featuredim, target_featuredim)
                loss_acceleration = loss_acceleration.mean() + self.loss(output_a_featuredim, target_featuredim)
