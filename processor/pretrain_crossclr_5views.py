...
class CrosSCLR_5views_Processor(PT_Processor):
    """
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label, frame in loader:#crop里面有random，后面再看，一个输入k一个输入q
            ...
            if epoch <= self.arg.cross_epoch:#没到cross的时候
                output, output_motion, output_bone, output_acceleration, output_rotation_axis, target = self.model(data1, data2, frame)#一个是q的输入，一个是k的输入
                ...
                loss = loss + loss_motion + loss_bone + loss_acceleration + loss_rotation_axis
            else:
                ...
                output_jr, output_rj, output_mr, output_rm, output_br, output_rb, output_ar, output_ra,\
                    ...
                    mask_jr, mask_rj, mask_mr, mask_rm, mask_br, mask_rb, mask_ar, mask_ra,\
                ...
                loss_jr = - (F.log_softmax(output_jr, dim=1) * mask_jr).sum(1) / mask_jr.sum(1)
                loss_rj = - (F.log_softmax(output_rj, dim=1) * mask_rj).sum(1) / mask_rj.sum(1)
                loss_mr = - (F.log_softmax(output_mr, dim=1) * mask_mr).sum(1) / mask_mr.sum(1)
                loss_rm = - (F.log_softmax(output_rm, dim=1) * mask_rm).sum(1) / mask_rm.sum(1)
                loss_br = - (F.log_softmax(output_br, dim=1) * mask_br).sum(1) / mask_br.sum(1)
                loss_rb = - (F.log_softmax(output_rb, dim=1) * mask_rb).sum(1) / mask_rb.sum(1)
                loss_ar = - (F.log_softmax(output_ar, dim=1) * mask_ar).sum(1) / mask_ar.sum(1)
                loss_ra = - (F.log_softmax(output_ra, dim=1) * mask_ra).sum(1) / mask_ra.sum(1)
                loss = (loss_jm + loss_jb + loss_ja + loss_jr) / 4.
                loss_motion = (loss_mj + loss_mb + loss_ma + loss_mr) / 4.
                loss_bone = (loss_bj + loss_bm + loss_ba + loss_br) / 4.
                loss_acceleration = (loss_ab + loss_aj + loss_am + loss_ar) / 4.
                loss_rotation_axis = (loss_rj + loss_rm + loss_rb + loss_ra) / 4.
                loss = loss.mean()
                loss_motion = loss_motion.mean()
                loss_bone = loss_bone.mean()
                loss_acceleration = loss_acceleration.mean()
                loss_rotation_axis = loss_rotation_axis.mean()
