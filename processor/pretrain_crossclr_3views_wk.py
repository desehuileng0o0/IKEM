...
class CrosSCLR_3views_WK_Processor(PT_Processor):
    """
    ...
    def train(self, epoch):
        ...
        for [data1, data2], label, frame in loader:#crop里面有random，后面再看，一个输入k一个输入q
            ...
            if epoch <= self.arg.cross_epoch:#没到cross的时候
                ...
                output_jm, output_jb, output_mj, output_mb, output_bj, output_bm, mask_jm, mask_jb, mask_mj, mask_mb, mask_bj, mask_bm,\
                    output_jr, output_mr, output_br, mask_jr, mask_mr, mask_br = self.model(data1, data2, frame, cross=True, topk=self.arg.topk, context=self.arg.context)
                if hasattr(self.model, 'module'):
                ...
                loss = (loss_jm + loss_jb + loss_jr) / 3.
                loss_motion = (loss_mj + loss_mb + loss_mr) / 3.
                loss_bone = (loss_bj + loss_bm + loss_br) / 3.
                loss = loss.mean()
